<template>
    <div class="bg-white h-12 px-4 flex items-center border-b border-gray-400 shadow-sm">
        <div class="w-1/3">
            <div class="flex">
                ...
                <div class="ml-2 relative">
                    ...
                    <input type="text" name="search" class="rounded-full pl-8 w-56 bg-gray-200 h-8 focus:outline-none focus:shadow-outline text-sm" placeholder="Search">
                </div>
            </div>
        </div>
        <div class="w-1/3 flex justify-center items-center h-full"  v-if="getAuthUser">
            <router-link to="/" class="px-6  h-full flex items-center " active-class="border-blue-500 border-b-2">
            ...
            <router-link :to="'/users/'+getAuthUser.user_id" class="px-6 h-full flex items-center" active-class="border-blue-500 border-b-2" >
            ...
            <router-link to="/messages" class="px-6 h-full flex items-center " active-class="border-blue-500 border-b-2">
        ...
        <div
            class="w-1/3 flex justify-end cursor-pointer"
            title="Logout"
            @click="logoutHandler"
        >
            <svg
