<template>
    <div class="bg-white rounded shadow m-2 mt-6 overflow-hidden">
            ...
            <div class="flex items-center">
                <div class="w-8">
                    <img
                        :src="
                            imagepath +
                            post.data.attributes.posted_by.data.profileimage
                ...
                <div class="ml-6">
                    ...
                    <div class="text-sm text-gray-600">
                        {{ post.data.attributes.post_created_date }}
                    </div>
                </div>
            </div>
            <div class="mt-4">
        ...
        <div class="w-full p-3" v-if="post.data.attributes.post_image">
            <img
                :src="post.data.attributes.post_image"
                alt="post image"
        ...
        <div class="px-4 pt-2 flex justify-between text-gray-700 text-sm">
            ...
            <div>
                <p
                    ...
                    @click="commentbutton = !commentbutton"
                >
        ...
        <div class="flex justify-between border-1 border-gray-400 m-4">
            <button
                @click="$store.dispatch('likepost', post.data.post_id)"
                class="flex justify-center py-2 rounded-lg text-sm text-gray-700 w-full hover:bg-gray-200"
                v-bind:class="{
                    'bg-blue-400 text-white': post.data.attributes.like_status,
                }"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
        ...
        <div v-if="commentbutton">
            <div
                ...
                class="w-full bg-white border-t border-gray-200 px-3 py-2"
            >
                <div class="flex justify-between items-center">
                    <div class="w-min-w">
                        <img
                            :src="
                                imagepath +
                                comment.data.attributes.data.posted_by.data
                                    .profileimage.data.attributes.image_path
                            "
                    ...
                    <div class="ml-2 w-full">
                        ...
                        <div class="flex text-sm text-gray-400 justify-between">
                            <span>{{
                                comment.data.attributes.data.created_at
                            }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex">
                    <div class="w-min-w">
                        <img
                            :src="
                                imagepath +
                                getAuthUser.profileimage.data.attributes
                                    .image_path
                            "
                            class="w-8 h-8 rounded-full object-cover"
                            alt=""
                    ...
                    <transition name="fade">
                        <div class="w-min-w" v-if="body.length > 0">
                            <button
                                @click="sendcomment()"
                                class="rounded-full bg-blue-500 border border-white text-white px-2 py-1"
                            >
                                SEND
                            </button>
                        </div>
...
export default {
    ...
    methods: {
        sendcomment() {
            axios
                .post("/api/posts/" + this.post.data.post_id + "/comments", {
